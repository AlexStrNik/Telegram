...
class ProfileActivityOverlaysView extends View implements ProfileGalleryView.Callback {

    private final ProfileActivity profileActivity;
    private int statusBarHeight() {
        return profileActivity.getActionBar().getOccupyStatusBar() && !profileActivity.isInBubbleMode() ? AndroidUtilities.statusBarHeight : 0;
    }
    ...
    public ProfileActivityOverlaysView(ProfileActivity profileActivity, Context context) {
        super(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        selectedPosition = profileActivity.avatarsViewPager.getRealPosition();
        ...
        if (count > 1 && count <= 20) {
            if (profileActivity.overlayCountVisible == 0) {
                alpha = 0.0f;
                profileActivity.overlayCountVisible = 3;
            } else if (profileActivity.overlayCountVisible == 1) {
                alpha = 0.0f;
                profileActivity.overlayCountVisible = 2;
            }
            if (profileActivity.overlayCountVisible == 2) {
            ...
            for (int a = 0; a < count; a++) {
                ...
                if (a == previousSelectedPotision && Math.abs(previousSelectedProgress - 1.0f) > 0.0001f) {
                    ...
                    if (profileActivity.avatarsViewPager.isCurrentItemVideo()) {
                        progress = currentProgress = profileActivity.avatarsViewPager.getCurrentItemProgress();
                        if (progress <= 0 && profileActivity.avatarsViewPager.isLoadingCurrentVideo() || currentLoadingAnimationProgress > 0.0f) {
                ...
                if (a != selectedPosition) {
                    if (profileActivity.overlayCountVisible == 3) {
            ...
            } else if (profileActivity.overlayCountVisible == 3) {
